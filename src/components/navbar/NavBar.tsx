...
export default function NavBar() {
  ...
  const NavLinks = ({ isMobile = false }) => (
    <ul className={`flex ${isMobile ? 'flex-col gap-8' : 'gap-1.5'}`}>
          ...
          <li key={index}>
            <FadeIn direction="down" delay={0.3 + index * 0.1} duration={0.4}>
              <Link
                ...
                className="text-white font-[400] flex items-center justify-center px-4 py-2 text-[16px] leading-[19.2px] hover:text-white/70 transition-colors duration-300 hover:cursor-pointer"
              >
                {link}
              </Link>
            </FadeIn>
  ...
  const GetStartedButton = () => (
    <FadeIn direction="left" delay={0.6} duration={0.5}>
      <HoverCard hoverScale={1.05} hoverElevation>
        <Button variant="customWithGradient" className="!h-[40px] sm:!h-[48px]">
          Get Started
        </Button>
      </HoverCard>
    </FadeIn>
  ...
  return (
    <FadeIn
      ...
      distance={20}
      duration={0.6}
      className={`fixed z-50 top-0 px-5 sm:px-8 py-4 w-full flex flex-col gap-[32px] items-center justify-center transition-all duration-300 ${
        isMobileMenuOpen ? 'bg-[#111111] rounded-bl-3xl rounded-br-3xl pb-10' : ''
      }`}
    >
      <div className="w-full flex items-center justify-center">
        <div className="bg-[#111111] flex items-center justify-between border border-[#FFFFFF14] w-[837.81px] rounded-full px-[9px] sm:px-5 py-[8px] sm:py-3">
          <FadeIn direction="right" delay={0.2} duration={0.5}>
            <Link href="/" className="font-bold flex items-center gap-3 text-[24px] leading-[100%] hover:cursor-pointer" onClick={e => handleScroll(e, 'home')}>
              <Image src={navLogo} width={48} height={48} className="w-[40px] sm:w-[48px] h-[40px] sm:h-[48px]" alt="Finabox logo" priority />

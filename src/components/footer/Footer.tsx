            ...
            <div className="w-full sm:w-[109px] flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-5">
                {quickLinks.map((link, index) => (
                  <FadeIn key={link} direction="right" delay={0.5 + index * 0.1} duration={0.4}>
                    <Link href={link === 'Home' ? '/' : `/${slugify(link)}`} className="text-white/50 text-base font-normal hover:text-white/70 transition-colors duration-300">
                      {link}
                    </Link>
                  </FadeIn>
                ))}
